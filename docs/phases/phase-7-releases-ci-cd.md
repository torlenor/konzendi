...
### Implementation decisions — 12 September 2026

The owner authorized pushing a branch, opening and merging a pull request, pushing `v0.1.0` as
the real first draft (left unpublished for the owner), and pushing throwaway failure-path tags
and a short-lived published test release, with cleanup afterwards. Implementation made these
choices within the accepted contract:

- **Smoke test in a clean container.** `scripts/package-smoke.sh` installs the `.deb` with
  `apt` in an `ubuntu:24.04` image pinned by digest, then runs the installed binary under
  `docker run --network none` on a private Xvfb display. Xvfb, xdotool, ImageMagick,
  `dbus-x11`, and `jq` are installed in that container, not on the runner. The runner already
...
- **CI-side logic in `scripts/release-ci.mjs`.** Asset collection, tag validation, and draft
  ...
  uploads, conflicts, and published releases.
- **Annotated tags are required.** `validate` refuses a lightweight tag with the command that
  creates the annotated one.
- **Increasing versions** are compared with published releases only. Drafts and unused failed
  tags do not block a version; the changelog check also requires descending dated entries.
- **Draft states.** A draft is titled *incomplete draft, do not publish* and carries a warning
  until all four assets are uploaded, downloaded again, and matched against the checked build;
  only then does it get its final title and notes.
- **Rehearsal fault injection.** The repository variable `RELEASE_FAULT` (`build` or `upload`)
  fails a tag run at a chosen point so reruns can be exercised without new commits or tags. It
  has no effect on pull request or `main` runs and is unset outside rehearsals.
- **Unreleased notes must already be complete.** `release:prepare` refuses Unreleased notes
  without `### Compatibility` and `### Known limitations`, so a prepared entry always passes
  `release:check`.
- **Third-party notices** are generated by `npm run notices` with cargo-about 0.9.2 (release
  binary pinned by SHA-256 in CI), run offline and locked for the Linux target without build or
  development dependencies. Procedural-macro crates and the application crate are excluded, and
  texts that differ only in white space are merged. `@tauri-apps/plugin-global-shortcut` ships
  no licence text; its MIT option is documented with the MIT text of `@tauri-apps/api` from the
  same project. The generated file is committed, installed to `/usr/share/doc/konzendi/`, and
  checked for drift in `package-smoke`. Crates under a licence outside `src-tauri/about.toml`
  stop generation until reviewed.
- **Package metadata.** `src-tauri/Cargo.toml` gains `authors = ["torlenor"]` and
  `publish = false`. Tauri adds `libayatana-appindicator3-1` to the package dependencies
  because of the tray icon.
