...
Track only questions that are unanswered, not handled by a phase, and not assigned to a phase.
Move a question into the relevant phase document when a phase takes ownership of it. Remove a
question when it is answered. The phase document keeps its decisions, rationale, and evidence.

Proposals in the [concept discussion](brainstorming/context-switching-app-concept.md) are inputs,
not defaults. A proposed option is not an accepted decision.

Question IDs stay stable. Do not renumber the remaining rows after a question is removed.

| ID | Decision needed | When needed | Current input |
| --- | --- | --- | --- |
